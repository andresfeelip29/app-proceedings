...
async function cargarUsuario() {
	...
	if (validador) {
		...
		const request = await fetch("/userRegister/userRegistration", {
			method: "POST",
			headers: {
		...
		if (request.ok) {
			...
			if (respuesta.username == 1 || respuesta.email == 1) {
				if (respuesta.username == 1) {
...
async function listarTipos() {
	...
	const request = await fetch("http://localhost/app_desarrollo_web/UserRegister/listAllUserType", {
		method: "GET",
	});
...
async function listarDependencias() {
	...
	const request = await fetch(
		"http://localhost/app_desarrollo_web/UserRegister/listAllProgramDependency",
		{
			method: "GET",
		}
	);
	const dependencias = await request.formData();
